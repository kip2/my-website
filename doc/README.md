...
記事はMarkdownファイルで別に用意しており、そのファイルをフェッチしてくる形式になっているため、記事をアップデートをした場合は、記事ファイルだけをサーバーに反映したら良い形式になっている。  
はずであった。

よくよく調査すると、どうやらページの更新をしただけでも毎回ビルドしなおす必要があることが判明。

そもそもの想定はgit pullして、記事ファイルのみ最新に反映させればページが更新され得るはずであった。

しかし、どうやら公開されているのはbuild後のdistディレクトリの記事ファイルであったようで、ソースディレクトリの記事ファイルをそちらにコピーする必要がある、ということがわかった。

というわけで、ページを単純にコピーするだけのシェルスクリプトを用意。
